...
class RidgeRegression(Model):
    ...
    def get_loss(self, V, q, m):
        Vhat, qhat, mhat = self._update_hatoverlaps(V, q, m)
        if self.data_model.commute:
            reg = np.mean((self.data_model.spec_Omega * qhat +
                        mhat**2 * self.data_model.spec_PhiPhit) /
                        (self.lamb + Vhat*self.data_model.spec_Omega)**2)
        else:
            reg = qhat * np.mean(self.data_model.spec_Omega / (self.lamb + Vhat*self.data_model.spec_Omega)**2)
            reg += mhat**2 * np.mean(self.data_model._UTPhiPhiTU /
                                   (self.lamb + Vhat * self.data_model.spec_Omega)**2)
        return self.alpha*(self.data_model.rho + q - 2*m) / (1+V)**2 + self.lamb*reg